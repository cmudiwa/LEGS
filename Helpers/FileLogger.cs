...
namespace ZimraEGS.Helpers
{
    public class FileLogger : ILogger
    {
        ...
        public void Log<TState>(LogLevel logLevel, EventId eventId, TState state, Exception exception, Func<TState, Exception, string> formatter)
        {
            ...
            var logRecord = $"{DateTime.UtcNow:yyyy-MM-dd HH:mm:ss} [{logLevel}] {_categoryName}: {formatter(state, exception)}";
            if (exception != null)
            {
    ...
    public class FileLoggerProvider : ILoggerProvider
    {
        ...
        {
            var logFilePath = Path.Combine(_logDirectory, $"log-{DateTime.UtcNow:yyyy-MM-dd}.log");
